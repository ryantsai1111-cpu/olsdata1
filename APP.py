        ...
        if st.button("📈 執行全量獨立分析並生成報告"):
            ...
            if all_results:
                ...
                st.dataframe(report_df.style.format({
                    ...
                    'P 值(P-value)': '{:.4f}', '判定係數(R-squared)': '{:.4f}', '調整後 R-squared': '{:.4f}',
                ...
                st.success("分析完成！已根據『解釋力 (R-squared)』自動排序，上方表格即為完整產出。")
            else:
    ...
    with tab6:
        ...
        if st.button("📊 產出 ANOVA 與詳細統計表"):
            ...
            if len(temp) > 10:
                ...
                model = sm.OLS(y, X).fit()
                st.markdown("#### 迴歸統計 (Regression Statistics)")
                st.table(pd.DataFrame({"指標": ["R Square", "Adjusted R Square", "Standard Error", "Observations"], "數值": [model.rsquared, model.rsquared_adj, np.sqrt(model.mse_resid), int(model.nobs)]}))
                st.markdown("#### 變異數分析 (ANOVA)")
